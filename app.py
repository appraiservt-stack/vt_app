...
def data_approx_all():
    ...
    for oid, entry in GEOCODED_APPROX.items():
        ...
        results.append({
            ...
            # Filterable fields — stored in cache so client-side filter works
            # without a separate ArcGIS fetch per approx dot.
            "price":   entry.get("ValPdOrTrn"),
            "date":    entry.get("closeDate"),   # epoch ms, same as /data
            ...
            # Filterable code fields
            "intPrpType":             entry.get("intPrpType"),
            "buildingConstruction1":  entry.get("blCn1"),
            "buildingConstruction2":  entry.get("blCn2"),
            "buildingConstruction3":  entry.get("blCn3"),
            "TownGrandListCategory":  entry.get("TownGlCat"),
            "sellerUseOfProperty":    entry.get("sUsePr"),
            "buyerUseOfProperty":     entry.get("bUsePr"),
            "propertyTaxExemption":   entry.get("prTxEx"),
            "landSize":               entry.get("landSize"),
            "sellerLastName": None,
